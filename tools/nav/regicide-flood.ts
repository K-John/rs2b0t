 ...
 *  Why: Tirannwn is a dozen sealed pockets, and "the pack says this tile is walkable" says nothing about
 *  whether the pocket standing on it can be walked to. */
...
for (const square of loadMapsquares(ENGINE)) {
    forEachLoc(new Reader(square.loc), loc => {
        ...
        // Why: a seam is only interesting when this pocket can stand next to it — and these are 3x2 locs, so
        // the tile the crossing is launched from is up to two off the origin on both axes.
        let touching = false;
