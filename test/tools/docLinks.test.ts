...
describe('extractRepoPaths', () => {
    test('finds backticked repo paths', () => {
        expect(extractRepoPaths('see `src/bot/engines/nav/PathFinder.ts` now').map(p => p.path)).toEqual(['src/bot/engines/nav/PathFinder.ts']);
    });
...
describe('resolveRelative', () => {
    test('resolves against the referring doc directory', () => {
        expect(resolveRelative('docs/NAV.md', '../src/bot/engines/nav/PathFinder.ts')).toBe('src/bot/engines/nav/PathFinder.ts');
    });
