...
import { UNSHEARED_SHEEP_ID, gatherWool, type WoolSites } from '#/bot/engines/quests/exec/wool.js';
...
describe('sheepshearer keeps its own sites and its own labels', () => {
    test('gatherBalls still returns the same step kinds and the Falador label', async () => {
        const { gatherBalls } = await import('#/bot/engines/quests/defs/sheepshearer.js');
