...
// get past this", and `arandar_gate` refuses southbound until the deed is reported.
 ...
 * Why: a failed pitfall jump teleports the player into mapsquare 36_150 with 15 damage, and the protruding
 * rocks are the only loc there — every other leg would read "unreachable" until this has run.
 */
 ...
 * Why: a component report over the quest's own anchors answers FAIL for every pair inside Tirannwn, so a
 * plain `walkResilient` there reports "unreachable" for anything past a crossing and the step reads as a
 * missing loc. Inside one pocket the navigator is still the right tool, which is why each round tries it.
 */
