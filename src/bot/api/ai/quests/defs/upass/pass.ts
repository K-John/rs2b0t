...
async function tryHops(
    ...
    for (const obstacle of list) {
        ...
        const op = kind.op;
        let now = from;
        // Why: every one of these refuses in words: "You can't do that from here", "I can't reach that!", "The rock is being used". A step that only reports "did not cross" hides which. Twenty ledge rolls at ninety-five per cent each cannot all fail, so the script was never running.
...
const USE_SEAMS: readonly UseSeam[] = [
    // Why: `@upass_rock_ropeswing` sets `$start_pos = 0_38_151_30_35`, meaning (2462,9699), and `~forcemove`s the character there before it rolls, then swings them four east to (2466,9699). The forcemove is a server walk: from a pocket that cannot reach that tile it never arrives, and the use reads as a seam that did nothing. Standing there first is what makes the roll the only thing left to fail.
    {
        item: UP_ITEM.ROPE,
