...
| 14 | `docs/CLUES.md` | 315 | reference | how-to (tracing, audit harness), explanation (yielding) | `docs/reference/clues.md` + `docs/how-to/trace-a-clue-failure.md` + `docs/decisions/clue-host-yielding.md` | ☐ |
...
**`docs/LOCS.md` is an external reference someone pasted in**, not authored for this repo, and its style is a game-design essay. It is linked from `docs/README.md`, so deleting it breaks nothing structurally. Decide: keep as-is and exempt, rewrite as reference, or delete.

**Root and package READMEs.** `README.md`, `desktop/README.md`, `templates/script-template/README.md` and `docs/nav/README.md` are entry points that conventionally mix a little of each type, and `test/docs/links.test.ts` asserts the first three exist by exact path. Treating them as strict single-type docs would either break the test or produce a front page that cannot orient a reader. Recommend exempting them from the one-type rule while still applying the line rules (no Overview, no banned words, no derivable content).
