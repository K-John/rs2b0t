...
[`specialCrossings.ts`](../../src/bot/engines/nav/data/specialCrossings.ts):
...
`src/bot/engines/nav/stateAwareRequires.ts` re-enables them with `requires` (merged at graph
