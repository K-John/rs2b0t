...
[`PathFinder`](../../src/bot/engines/nav/PathFinder.ts) is A\* over the pack, running inside a
worker ([`NavWorker.ts`](../../src/bot/engines/nav/NavWorker.ts)) so a long search never stalls
the game loop. [`Navigator`](../../src/bot/engines/nav/Navigator.ts) is the front end:
...
[`dangerZones.ts`](../../src/bot/engines/nav/data/dangerZones.ts); pass them on any walk
...
`test/engines/nav/dangerZones.test.ts`.
...
[`DirectNavigator`](../../src/bot/engines/nav/DirectNavigator.ts) is the script-facing wrapper —
see [Movement](../reference/api-movement.md).

## Following a path

[`WalkExecutor`](../../src/bot/engines/nav/WalkExecutor.ts) turns waypoints into clicks. Each
pass it:
...
The pure geometry is split into [`followMath.ts`](../../src/bot/engines/nav/geometry/followMath.ts) so it
can be unit-tested without a client — [`test/engines/nav/followMath.test.ts`](../../test/engines/nav/followMath.test.ts)
