...
The painted webwalker route and the tiles the client walks after a click come
...
1. **Different maps.** The pack is a full-world snapshot; the client only knows the loaded scene.
2. **Chebyshev expand overshoots.** `expandChebyshevSegment` uses `sign(dx)*step` for `max(|dx|,|dz|)` steps, so uneven diagonals overshoot the waypoint. Pinned in `pathExpand.test.ts`.
3. **Click horizon.** The walker clicks ~20 path steps ahead; the client may route only partway.
4. **Projection is camera-aligned.** `pathScenePaint` projects the route correctly; the divergence is in the tile sequence.
