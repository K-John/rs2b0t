...
- **A distance test cannot answer a question about topology, and the second cavern asks five of them.**
  ...
  server refusal such as "I can't reach that!" or "You can't do that from here" is not one. Which of a door's four
...
- **The offline seam graph cannot be transcribed into a route.** Every crossing that is not a loc op is
  absent from it, meaning the spade dig, the rope swing and both telejumps, so a breadth-first search over the
