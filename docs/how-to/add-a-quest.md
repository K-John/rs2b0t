...
1. Add the record to [`data/quests.ts`](../../src/bot/engines/quests/data/quests.ts) — id, name, quest points, requirements, items.
...
3. Register it in [`defs/index.ts`](../../src/bot/engines/quests/defs/index.ts).
4. Add a unit test for `decide()` — it is a pure function, so every branch is
   testable without a client. See [`test/engines/quests/`](../../test/engines/quests/).
...
Start from [`defs/cooksassistant.ts`](../../src/bot/engines/quests/defs/cooksassistant.ts) for
the simple shape, [`defs/priestperil.ts`](../../src/bot/engines/quests/defs/priestperil.ts)
for one with level changes, gated doors, and a long item chain, or
[`defs/watchtower/`](../../src/bot/engines/quests/defs/watchtower/) for one large enough to need a
directory.
